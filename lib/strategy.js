...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    this._secretOrKeyProvider(req, token, function(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            ...
            // callback for the parsed token to the user
            ...
            if (Array.isArray(secretOrKey)) {
                // verify a list of public keys

                const jwtVerifierPromise = (token, curKey, verifOpts) => {
                    return new Promise((resolve, reject) => {
                        JwtStrategy.JwtVerifier(token, curKey, verifOpts, function(jwt_err, payload) {
                            if (jwt_err) {
                                return resolve({err:jwt_err, result: false})
                            }
                            resolve({payload:payload, result: true})
                        })
                    })
                }

                co(function* () {
                    var keyList = secretOrKey
                    var keyCount = 0
                    var verifierResult
                    var successKey = false
                    
                    while (keyCount<=keyList.length) {
                        verifierResult = yield jwtVerifierPromise(token, keyList[keyCount], self._verifOpts)
                        if (verifierResult.result) {
                            successKey = true
                            try {
                                if (self._passReqToCallback) {
                                    self._verify(req, verifierResult.payload, verified)
                                } else {
                                    self._verify(verifierResult.payload, verified)
                                }
                            } catch(ex) {
                                self.error(ex)
                            }
                    ...
                    if (!successKey) {
                        return self.fail(verifierResult.payload)
                    }
